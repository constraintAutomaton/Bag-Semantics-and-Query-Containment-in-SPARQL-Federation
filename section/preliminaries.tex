...
\acp{UCQ} \rt{Several spelling mistakes here} are formed by taking the union of multiple \acp{GP} via a \ac{UGP}, allowing a query to match any of several alternative patterns.

Matching a graph pattern \rt{What is a graph pattern? Do you mean a BGP?} against an \ac{RDF} graph produces \emph{solution mappings}, partial functions from variables to \ac{RDF} terms.
\rt{Let's also mention solution sequence here.}
...
We denote the evaluation of a query over a \ac{KG} $G$ as $\eval{Q}{G}$ \rt{This first part probably should have come before already}, and the evaluation over a federation of endpoints $F$ as $\eval{Q}{F}$, where $F$ is defined as a set of federation members
...
formalized as a mapping $\mathit{fm}\colon \mathit{IRI} \mapsto G$.\rt{Endpoints should be dereferenceable, so let's use URL instead of IRI.}
...
We distinguish two source-selection mechanisms for federated queries: \ac{UDF} and \ac{ASSF}~\cite{Cheng2021} \rt{I strongly recommend using automatic source selection and manual source selection}.
Under \ac{UDF}, the query uses \texttt{SERVICE} clauses~\cite{w3SPARQLFederated} that name the target endpoint of each subquery; in this paper, we restrict attention to \texttt{SERVICE} clauses whose target is a constant \ac{IRI}\rt{Let's add a footnote explaining that this is the only thing that the spec is normative about, and most engines actually do. And that the alternative would be to also support variables, which is non-normative in the spec.}.
...
\rt{Do we really need these extra SSA and DSA concepts? Can we not just only keep automatic and manual source selection/assignment? All these abbreviations make the paper very hard to read.}
