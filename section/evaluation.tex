...
\Cref{fig:all_suites_bfc_specs} shows that the times of \solverName{} spread further across pairs.
Its head ratio, the median over the 5th percentile, is 1.9 against 1.1 for SpeCS, and its tail ratio, the 95th percentile over the median, is 56.6 against 17.5.
Across repeated runs of the same pair, SpeCS varies more than \solverName{} on the pairs both answer correctly ($p = 2.48 \times 10^{-40}$).
...
On the operators and star suites its median stays close to that of SpeCS, while its times spread further down, as \solverName{} settles many pairs with its necessary conditions, which are polynomial, or with a containment mapping (\Cref{sec:resolution}), without resorting to an SMT solver.
