...
\Cref{tab_decidable_containment_bag_bag_set} presents query fragments with decidable bag and bag-set query containment.\footnote{Inspired by \citeauthor{Afrati2010}~\cite{Afrati2010}.}

\begin{table*}
  \caption{Decidable bag and bag-set query fragments.}
  \label{tab_decidable_containment_bag_bag_set}
  \begin{tabular}{llll}
    \toprule
    Case & Semantic & Time Complexity & Necessary Condition\\
    \midrule
    General \ac{CQ}~\cite{Chaudhuri1993} & Bag and Bag-set & $\Pi^p_2$-hard & \textbf{No}\\

    \ac{CQ} where $Q_1$ is without projections~\cite{Afrati2010} & Bag-set & NP-complete & Yes\\
    \ac{CQ} where $Q_1$ is without projections~\cite{Konstantinidis2019} & Bag & $\Pi^p_2$ & Yes\\
    ...
    Pathstar \ac{CQ}~\cite{Afrati2010} & Bag and Bag-set & $O(n^2\log(n))$ & Yes\\
    Simple general Pathstar \ac{CQ}~\cite{Afrati2010} & Bag and Bag-set & $O(n)$ & Yes\\
    $Q_1$ is an enhanced version of $Q_2$~\cite{Afrati2010} & Bag-set & $O(n)$ & Yes\\
    \bottomrule
  \end{tabular}
\end{table*}
