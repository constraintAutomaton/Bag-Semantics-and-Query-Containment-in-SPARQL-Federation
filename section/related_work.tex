...
\begin{table*}
  \caption[Decidable bag and bag-set query fragments]{Decidable bag and bag-set query fragments\protect\footnote{Inspired by \citeauthor{Afrati2010}~\cite{Afrati2010}.}}
  ...
  \begin{tabular}{llll}
    ...
    \ac{CQ} where $Q_1$ and $Q_2$ are without projections~\cite{Afrati2010} & Bag-set & $O(n^2\log(n))$ & Yes\\
    \ac{CQ} where $Q_1$ and $Q_2$ are without projections~\cite{Afrati2010} & Bag & $O(n^2\log(n))$ & Yes\\
...
Automatic source selection provides algorithms that alleviate the practical difficulty of determining which federation member each subquery should be sent to~\cite{hanski2025observations}.
Thus, users can query a federation without knowing how the data is distributed.
Furthermore, \citeauthor{Cheng2021}~\cite{Cheng2021} introduces \ac{ESA}, a canonical source-selection algorithm that requires no prior information about the federation.
