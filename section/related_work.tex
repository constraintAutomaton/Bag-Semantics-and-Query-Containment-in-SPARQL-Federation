...
\begin{table*}
  \caption{Decidable bag and bag-set query fragment\protect\footnote{Inspired from \citeauthor{Afrati2010}~\cite{Afrati2010}.}}
  ...
  \begin{tabular}{llll}
    ...
    \ac{CQ} where $Q_1$ and $Q_2$ are without projections~\cite{Afrati2010} & Bag-set & $O(n^2\log(n)\times n_{sQ1} \times n_{sQ2})$ & Yes\\
    \ac{CQ} where $Q_1$ and $Q_2$ are without projections~\cite{Afrati2010} & Bag & $O(n^2\log(n)\times n_{sQ1} \times n_{sQ2})$ & Yes\\
    ...
    $Q_2$ is an enhanced version of $Q_1$~\cite{Afrati2010} & Bag-set & $O(n)$ & Yes\\
    \bottomrule
  \end{tabular}
\end{table*}
...
\citeauthor{Cheng2021}~\cite{Cheng2021} proposed FedQPL, a formalism to express federated queries under \ac{ASSF}, addressing the difficulty of manually determining which federation members to query for a given graph pattern~\cite{hanski2025observations}.
