...
\subsection{Bag Federation Reduction}\label{sec_bag_federation_reduction}

\begin{definition}\label{def_bkg}
A \ac{BKG} is defined by a special bag union of all \ac{KG} of each federation member.
This special bag union gives a multiplicity $m_{\mathrm{BKG}}$ given the multiplicity $m_{\mathrm{bag}}$ of a standard bag union define by
\[
m_{\mathrm{BKG}} =
\begin{cases}
1 & \text{if } m_{\mathrm{bag}} = 1,\\
x & \text{if } m_{\mathrm{bag}} > 1,
\end{cases}
\quad \text{for some } x \in \mathbb{N}_{\geq 1}
\]
\end{definition}
...
The exact bag multiplicity is left unspecified beyond being greater than one, which can appear problematic, however, the \ac{BKG} is a fictional construction used only to determine the semantics of the query.
As long as the multiplicity of a solution mapping related to a triple is greater than one in a conventional federated query and the corresponding multiplicity is also greater than one in the \ac{BKG}, the construction is sound for our purpose.
This is particularly useful given that \ac{UCQ} containment under bag-set and set semantics is undecidable, as discussed in \Cref{sec:related_works_qc}.
...
There exist a \ac{BKG} for which the query performed over a federation where all members host identical \ac{KG} produce the same results in this \ac{BKG}.
\end{proposition}

\begin{proof}
When all $m$ members host identical \ac{KG}, every $\freq_{\mathit{fm}_i}^{\mathit{tp}_j}$ produces the same result for a given $\mathit{tp}_j$.
...
By definition of the \texttt{JOIN} the multiplicity will $m^2$ times the number of times solution mappings can be merged.

In the \ac{BKG}, the special bag union of $m$ identical \acp{KG} can be given each triple a multiplicity of $m$, so evaluating the query over the \ac{BKG} produces the same multiplicities.
...
The \emph{Bag Federation Reduction} thus covers the full spectrum of union-based federation strategies.
