...
$\Omega_1$ and and $\Omega_2$ are the two first union of the equation.

We are investigating the max of $mu$ because given the data is identitcal inside of the $\Omega_i$, either the data can me merged fully or no results are produced
\begin{align*}
\max(|m(\mu)|) = \max(\{(\mu_1, \mu_2) | \operatorname{merge}(\mu_1 \in \Omega_1, \mu_2 \in \Omega_2) = \mu\}) \\
= m^3
\end{align*}

for \ac{BKG}, there is no union involved.
$\freq_{\mathit{fm}_i}^{\mathit{tp}_j}$ has a multiplicity of $m$ given in the \ac{BKG} the data is multiplied by $m$
...
for \ac{BKG} there is no union so we can start with the join.

the mu remain the same.
